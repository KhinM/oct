...
start.addEventListener("click",
  function() {
    ...
    soundControl("start", "start.mp3"); //サウンド
    soundEndflag = "1";
...
stop.addEventListener("click",
  function() {
    ...
    if(timer.textContent.substring(0,5) === "00:10"){
      soundControl("start", "stop2.mp3"); //サウンド
      //backgroundの設定
      const fireworks = document.getElementsByTagName("body");
      fireworks[0].style.backgroundImage = "url('fireworks.gif')";//花火
      fireworks[0].style.backgroundColor = "rgba(0, 0, 0, 0)";
    }
    else{
      soundControl("start", "stop1.mp3"); //サウンド
    }
  },false
);
...
reset.addEventListener("click",
  function() {
    ...
    soundControl("start", "reset.mp3"); //サウンド
    soundEndflag = "1";
    // ボタンを"初期"状態とする
    setButtonStateInitial();
